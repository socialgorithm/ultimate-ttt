...
function startSession(session, settings){
  ...
  function handleGameEnd(winner, playerDisconnected) {
    console.log(server.game.stateBoard.prettyPrint());
  ...
  function playGame() {
    ...
    server.state.games++;
    console.log('Game ' + server.state.games + '/' + server.maxGames + '');
  ...
  function handlePlayerMove(player) {
    return function(data) {
      if (server.currentPlayer !== player) {
        console.log('Player ' + player + ' played out of time (it was ' + server.currentPlayer + ' turn)');
